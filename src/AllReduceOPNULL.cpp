...
int main(int argc, char *argv[]) {
    ...
    for(int i = 0; i < BENCHMARK_ITERATIONS + WARM_UP; ++i){
        ...
        //MPI_Allreduce(send_buffer, recv_buffer, msg_count, MPI_FLOAT, noop_op, MPI_COMM_WORLD);
        allreduce_ring(send_buffer, recv_buffer, msg_count, MPI_FLOAT, noop_op, MPI_COMM_WORLD);
    ...
    cout << rank << " : "<< processor_name <<" : ALL REDUCE OP NULL -> Buffer: "  << BUFFER_SIZE << " byte - " << buffer_gib << " Gib - " << mib_count << " MiB, verifier: " << verifier << ", Latency: " << total_time << ", Bandwidth: " << bandwidth << endl;
    //}
