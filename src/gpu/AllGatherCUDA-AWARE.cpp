...
int main(int argc, char *argv[]) {
    ...
    size_t BUFFER_SIZE_SMALL = (size_count * multiplier_type)/size;
    long long int msg_count = BUFFER_SIZE_SMALL/sizeof(int);
    int *h_send_buffer = (int*) malloc(BUFFER_SIZE_SMALL); 
    int *h_recv_buffer = (int*) malloc(BUFFER_SIZE);


    int *d_send_buffer;
    CUDA_CHECK(cudaMalloc((void**)&d_send_buffer, (size_t) BUFFER_SIZE/size));
    int *d_recv_buffer;
    ...
    CUDA_CHECK(cudaMemcpy(d_send_buffer, h_send_buffer, (size_t) BUFFER_SIZE_SMALL, cudaMemcpyHostToDevice));
    ...
    for(int i = 0; i < BENCHMARK_ITERATIONS + WARM_UP; ++i){
        ...
        MPI_Allgather(d_send_buffer, msg_count, MPI_INT, d_recv_buffer, msg_count, MPI_INT, MPI_COMM_WORLD);
