...
double allreduce_swing_bdw_mesh(const void *send_buf, void *recv_buf, size_t count,
  ...
  for(step = 0; step < steps; step++) {
    ...
    for (size_t seg = 0; seg < num_segments; ++seg) {
      ...
      current_segment_size = min(segment_size, s_count[step] - offset); //segment_size; //min(segment_size, s_count[step] - offset);
      /*
...
int allreduce_swing_bdw_mesh(const void *send_buf, void *recv_buf, size_t count,
  ...
  for(step = 0; step < steps; step++) {
    ...
    for (size_t seg = 0; seg < num_segments; ++seg) {
      ...
      MPI_Recv(tmp_buf, current_segment_size, dtype, dest, seg, comm, MPI_STATUS_IGNORE);
      tmp_recv = (char *) recv_buf + r_index[step] * datatype_size + offset * datatype_size;
      ...
      if(seg + 1 < num_segments) {
        ...
        MPI_Isend(tmp_send + offset_send * datatype_size, current_segment_size_send, dtype, dest, seg+1, comm, &send_req_pipe[seg+1]);
      }
    }
