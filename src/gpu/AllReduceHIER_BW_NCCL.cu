...
inline int log_2(int value) {
  ...
  if (1 > value) {
      ...
      QUALCOSA NON TORNA NEL VALORE PASSATO, VALUTARE
  */
...
int allreduce_swing_bdw_mesh(const void *send_buf, void *recv_buf, size_t count,
  ncclDataType_t dtype, ncclRedOp_t op, ncclComm_t comm, cudaStream_t stream, uint *peers, swing_tree_t *tree, char *tmp_buf_raw, size_t segment_size){
  ...
  datatype_size = sizeof(dtype);

  // Does not support non-power-of-two or negative sizes
  steps = log_2(size);

  segment_size = segment_size / datatype_size;
  ...
  for(step = 0; step < steps; step++) {
    ...
    ncclGroupStart();
    ncclSend(tmp_send + 0, segment_size, dtype, dest, comm, stream_send);
    for (size_t seg = 0; seg < num_segments; ++seg) {
      ...
      ncclRecv(tmp_buf, current_segment_size, dtype, dest, comm, stream_recv);
      tmp_recv = (char *) recv_buf + r_index[step] * datatype_size + offset * datatype_size;

      cerr << "porcodioLOOP" << endl;
      
      if(step == 0){        
        char* tmp_send_0 = (char *) send_buf + r_index[step] * datatype_size + offset * datatype_size;
        reduce_sum_kernel_step0<<<512, 512, 0, stream_recv>>>((const int*)tmp_buf, (int*)tmp_send_0, (int*)tmp_recv, current_segment_size);
      } else {        
        reduce_sum_kernel<<<512, 512, 0, stream_recv>>>((const int*)tmp_buf, (int*)tmp_recv, current_segment_size);
      }
      ...
      if(seg + 1 < num_segments) {
        ...
        ncclSend(tmp_send + offset_send * datatype_size, current_segment_size_send, dtype, dest, comm, stream_send);
      }
    ...
    cudaStreamSynchronize(stream_recv);
  ...
  */
  ...
  for(step = 0; step < steps; step++) {
    ...
    ncclGroupStart();
    ncclRecv(tmp_buf, r_count[step], dtype, dest, comm, stream_recv);
    ncclSend(tmp_send, s_count[step], dtype, dest, comm, stream_send);
    ncclGroupEnd();

    if(step == 0){
      tmp_recv = (char *) recv_buf + r_index[step] * datatype_size;
      char* tmp_send_0 = (char *) send_buf + r_index[step] * datatype_size;
      reduce_sum_kernel_step0<<<512, 512, 0, stream_recv>>>((const int*)tmp_buf, (int*)tmp_send_0, (int*)tmp_recv, r_count[step]);
    } else {
      tmp_recv = (char *) recv_buf + r_index[step] * datatype_size;
      reduce_sum_kernel<<<512, 512, 0, stream_recv>>>((const int*)tmp_buf, (int*)tmp_recv, r_count[step]);
    }
  ...
  for(step = steps - 1; step >= 0; step--) {
    ...
    ncclGroupStart();
    ncclSend(tmp_send, r_count[step], dtype, dest, comm, stream_send);
    ncclRecv(tmp_recv, s_count[step], dtype, dest, comm, stream_recv);
    ncclGroupEnd();
  }
  ...
  cudaStreamSynchronize(stream_send);
  cudaStreamDestroy(stream_recv);
  cudaStreamDestroy(stream_send);
...
int intra_reducescatter_block(void *sendbuf, void *recvbuf, size_t recvcount, ncclDataType_t recvtype, ncclComm_t comm, cudaStream_t stream){
    // Do a reduce-scatter where each rank isends and irecvs from everyone else
    int rank, size;
    ncclCommUserRank(comm, &rank);
    ncclCommCount(comm, &size);
    int datatype_size = sizeof(recvtype);
    ...
    for (int i = 0; i < size; i++) {
        if (i != rank) {
          ncclGroupStart();
          ncclSend((char*) sendbuf + i * recvcount * datatype_size, recvcount, recvtype, i, comm, stream_send);
          ncclRecv((char*) recvbuf + i * recvcount * datatype_size, recvcount, recvtype, i, comm, stream_recv);
          ncclGroupEnd();
        }
    } 
    cudaStreamSynchronize(stream_recv);
    sum4arrays<<<512, 512, 0, stream_recv>>>((const int*) ((rank == 0 ? (char*) sendbuf : (char*) recvbuf) + 0 * recvcount * datatype_size), 
...
int intra_reducescatter_block_segmented(void *sendbuf, void *recvbuf, size_t recvcount, ncclDataType_t recvtype, ncclComm_t comm, cudaStream_t stream_base, size_t segment_size) {
  int rank, size;
  ncclCommUserRank(comm, &rank);
  ncclCommCount(comm, &size);
  ...
  for (int seg = 0; seg < num_segments; ++seg) {
      ...
      for (int peer = 0; peer < size; ++peer) {
          if (peer != rank) {
              ...
              //ncclGroupStart();
              ncclSend((char*)sendbuf + offset, curr_seg_count, recvtype, peer, comm, stream_send);
              ncclRecv((char*)recvbuf + offset, curr_seg_count, recvtype, peer, comm, stream_recv);
              //ncclGroupEnd();
  ...
  for (int seg = 0; seg < num_segments; ++seg) {
    int curr_seg_count = (seg == num_segments - 1) ? last_segment_count : segment_count;
      ...
      sum4arrays<<<512, 512, 0, stream_recv>>>(a, b, c, d, out, curr_seg_count);
  }
...
int intra_allgather(void *recvbuf, size_t recvcount, ncclDataType_t recvtype,
    ...
    for (int i = 0; i < size; i++) {
        if (i != rank) {
            ncclGroupStart();
            ncclSend(((char*) recvbuf) + rank * recvcount * datatype_size, recvcount, recvtype, i, comm, stream_send);
            ncclRecv(((char*) recvbuf) + i * recvcount * datatype_size, recvcount, recvtype, i, comm, stream_recv);
            ncclGroupEnd();
        }
    }
...
#define GPUS_PER_NODE 4    
    ...
    for (size_t i = 0; i < msg_count; i++) {
        h_send_buffer[i] = rank;//rand()*rank % 10; 
    }
    ...
    ncclComm_t intra_comm, inter_comm;
    ncclCommSplit(comm, (rank / GPUS_PER_NODE), rank, &intra_comm, NULL);
    ncclCommSplit(comm, (rank / GPUS_PER_NODE), rank, &inter_comm, NULL);
    int intra_rank, inter_rank;
    int intra_size, inter_size;
    ncclCommCount(intra_comm, &intra_size);
    ncclCommCount(inter_comm, &inter_size);
    ncclCommUserRank(intra_comm, &intra_rank);
    ncclCommUserRank(inter_comm, &inter_rank);
    ...
    SwingCoordConverter* scc = new SwingCoordConverter(new uint[1]{ (uint) (size / GPUS_PER_NODE)}, 1); 
    ...
    for(int i = 0; i<msg_count; i++){
      cout << "START rank" << rank << " index: " << i << " value: " << h_send_buffer[i] << endl;  
    }
    ...
    // This is it
    cerr << "EI" << endl;
    intra_reducescatter_block(d_send_buffer, d_recv_buffer, msg_count / GPUS_PER_NODE, ncclInt, intra_comm, stream);    
    //intra_reducescatter_block_segmented(d_send_buffer, d_recv_buffer, msg_count / GPUS_PER_NODE, ncclInt, intra_comm, stream, hier_segment_size);    
    // d_recv_buffer is large enough, I can use part of it as recvbuf
    cerr << "AO" << endl;
    allreduce_swing_bdw_mesh(redscat_out_buf, allreduce_out_buf, (msg_count / GPUS_PER_NODE), ncclInt, ncclSum, inter_comm, stream, peers, &tree, tmp_buf, inter_segment_size);
    // Now I can do an allgather on the intra communicator
    cerr << "EO" << endl;
    intra_allgather(d_recv_buffer, (msg_count / GPUS_PER_NODE), ncclInt, intra_comm, stream);
    cerr << "PORCOCAZZO" << endl;

    ncclAllReduce(d_send_buffer, d_test_recv_buffer, msg_count, ncclInt, ncclSum, comm, stream);
    ...
    for(int i = 0; i<msg_count; i++){
      cout << "END rank" << rank << " index: " << i << " value: " << h_recv_buffer[i] << endl;  
    }
